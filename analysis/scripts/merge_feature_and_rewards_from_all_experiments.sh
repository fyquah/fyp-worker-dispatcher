...
for exp in $(python scripts/query_experiment_params.py --all-old); do
  ...
  ./_build/default/tools/local_reward_model.exe plots dump-data \
    ...
    -spec $TMP \
    >w/reward-$REWARD_VERSION-feature-$FEATURE_VERSION/$REWARD_MODEL/$exp/feature_reward_pair.sexp
  ...
  cp w/reward-$REWARD_VERSION-feature-$FEATURE_VERSION/$REWARD_MODEL/$exp/feature_reward_pair.sexp \
