...
for problem in $PROBLEMS; do
  echo "./learn_linear_general_reward_without_normalisation.sh out-$VERSION/$problem &>tmp/$VERSION-$problem-learn-linear-general-reward-without-normalisation.log" >>$TASK_FILE
done

echo "Task file: $TASK_FILE" >/dev/stderr

parallel --verbose -t <$TASK_FILE
