...
let construct_tf_model ~(hyperparams : Tf_helper.hyperparams) num_features =
  ...
  let target_placeholder =
    O.placeholder [ -1; num_outputs; ] ~type_:Float
  in
...
let filter_eligible_examples examples =
  List.filter_map examples
    ~f:(fun (feature, (opt_dual_reward, opt_no_inline)) ->
      match Option.both opt_dual_reward opt_no_inline with
      | None -> None
      | Some (dual_reward, no_inline) ->
...
let create_model ~hyperparams (examples: example list) =
  ...
  let raw_targets  =
    Array.of_list_map examples ~f:snd
  ...
  let target_matrix = Owl.Mat.of_arrays raw_targets in
...
let do_analysis (examples : example list)
    ~dump_normaliser ~checkpoint ~dump_graph ~hyperparams ~epochs ~(test_examples : example list) =
...
let command =
  ...
  Command.async
    ~summary:"Model for predicting short term rewards of an inlining decision"
    [%map_open
      let {
        specification_file;
        epochs;
